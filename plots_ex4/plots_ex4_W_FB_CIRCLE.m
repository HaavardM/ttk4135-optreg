...
run ex4feedback_circle;
close;

%Getting data from file
filename = '../data_ex4/ex4_q1_10_q2_2_w_feedback_CIRCLE.mat';
m = matfile(filename);
data = m.ans;

%Data from encoders
time = data(1,:);
encoder_travel = data(2,:);
...
axis([0,min(time(1,length(time)),opt_time(1,length(opt_time))), min(min(opt_travel),min(encoder_travel))-10, max(max(opt_travel),max(encoder_travel))+10]);
...
axis([0, min(time(1,length(time)),opt_time(1,length(opt_time))), min(min(opt_travel_rate),min(encoder_travel_rate))-10, max(max(opt_travel_rate),max(encoder_travel_rate))+10]);
...
axis([0,min(time(1,length(time)),opt_time(1,length(opt_time))), min(min(opt_pitch),min(encoder_pitch))-10, max(max(opt_pitch),max(encoder_pitch))+30]);
...
axis([0, min(time(1,length(time)),opt_time(1,length(opt_time))), min(min(opt_pitch_rate),min(encoder_pitch_rate))-10, max(max(opt_pitch_rate),max(encoder_pitch_rate))+10]);
...
axis([0,min(time(1,length(time)),opt_time(1,length(opt_time))), min(min(opt_elevation),min(encoder_elevation))-10, max(max(opt_elevation),max(encoder_elevation))+30]);
...
hold;
stairs(opt_time, opt_elevation);
axis([0, min(time(1,length(time)),opt_time(1,length(opt_time))), min(min(opt_elevation_rate),min(encoder_elevation_rate))-10, max(max(opt_elevation_rate),max(encoder_elevation_rate))+10]);
